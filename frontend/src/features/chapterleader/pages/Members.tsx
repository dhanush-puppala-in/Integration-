...
// Lottie Animations
import earthLottie from "../../../assets/Earth_Animation.json";
import planetOrbitLottie from "../../../assets/Planet_Orbit.json";
import planetStarsLottie from "../../../assets/Planet_Stars.json";
import planetLoaderLottie from "../../../assets/Plant_Loader.json";
import redPlanetLottie from "../../../assets/Red_Planet.json";
import rotatingPlanetLottie from "../../../assets/Rotating_Planet_Loader.json";
...
const Members: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const res = await chapterLeaderApi.getQuestsProgress('Member');
        console.log(res)
  ...
  if (stages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-white w-full relative">
        <Starfield />
        <StickyHeader currentStage={undefined as any} label="Members Guild" />
        <div className="flex-1 flex flex-col items-center justify-center px-6 relative z-10 text-center">
          <div className="w-24 h-24 mb-8 bg-white/5 rounded-full flex items-center justify-center border border-white/10 animate-pulse">
            <GiGalaxy className="text-5xl text-gray-500" />
          </div>
          <h2 className="text-3xl font-black uppercase tracking-tighter mb-4 text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500">
            Deep Space Silence
          </h2>
          <p className="max-w-md text-gray-400 text-lg font-medium italic">
            "No member orbits have been established yet. We're currently aligning the stars for your next recruitment drive."
          </p>
          <div className="mt-12 flex flex-col items-center gap-4">
            <div className="h-px w-24 bg-gradient-to-r from-transparent via-white/20 to-transparent" />
            <span className="text-[10px] font-black tracking-[0.4em] text-gray-600 uppercase">
              Awaiting neural link activation
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center min-h-screen text-white w-full relative">
      ...
      {/* --- STICKY HEADER --- */}
      <StickyHeader currentStage={currentStage} label="Members Guild" />
      {/* --- MISSION MAP --- */}
